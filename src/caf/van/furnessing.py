...
def compare_totals(matrix: np.ndarray, col_total: np.ndarray, row_total: np.ndarray) -> float:
...
def furness_2d(
    ...
    while diff > diff_cutoff:
        ...
        if not np.all(np.isfinite(matrix)):
            msg = "Matrix contains non-finite values, " f"stopping furnessing on loop {loop}"
            break
    else:
        converged = True
        msg = f"Furness converged (loop {loop}) " f"with RMS row/column difference: {diff:.1e}"
